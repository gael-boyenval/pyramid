...
var parseQueryList = function parseQueryList(queryList) {
  ...
  _postcss.list.comma(queryList).forEach(function (query) {
    var expressions = {};

    _postcss.list.space(query).forEach(function (expression) {
      ...
      newExpression = _postcss.list.split(
        newExpression.replace(/^\(|\)$/g, ''),
        [':'],
      );
...
var pickMinimumMinWidth = function pickMinimumMinWidth(expressions) {
  ...
  expressions.forEach(function (feature) {
    ...
    minWidths.push(
      minWidth.map(inspectLength).sort(function (a, b) {
        return b - a;
      })[0],
    );
  });
...
function addToAtRules(node) {
  ...
  if (node.parent.type !== 'root') {
    var newAtRule = _postcss.default.atRule({
...
var _default = _postcss.default.plugin('pyramid', function (options) {
