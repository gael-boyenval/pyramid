...
var messages = _stylelint.default.utils.ruleMessages(ruleName, {
  expectedSyntax: function expectedSyntax(substring, selector, caseStyle) {
    return 'Expected "'
      .concat(substring, '" in ".')
      .concat(selector, '" to be "')
      .concat(caseStyle, '"');
  },
});
...
var match = function match(part, configCase) {
...
var checkCase = function checkCase(
  decomposedSelector,
  rule,
  result,
  ruleName,
  ...
  decomposedSelector.forEach(function (selectorPart) {
    if (selectorPart.type === 'class') {
      selectorPart.bemStructure.forEach(function (classPart) {
        var isValidCase =
