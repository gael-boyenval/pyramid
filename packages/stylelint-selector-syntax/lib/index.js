...
var _checkSelectorComposition = _interopRequireDefault(
  require('./checkSelectorComposition'),
);

var _checkPropsValueAtBlockLevel = _interopRequireDefault(
  require('./checkPropsValueAtBlockLevel'),
);
...
var isStringOrRegExp = function isStringOrRegExp(x) {
  return _.isString(x) || _.isRegExp(x);
};

var executeSelectorChecks = function executeSelectorChecks(
  decomposedSelector,
  rule,
  result,
  ruleName,
  ...
  (0, _checkSelectorComposition.default)(
    decomposedSelector,
    rule,
    result,
    ruleName,
...
var isBlockLevelSelector = function isBlockLevelSelector(selector) {
  return (
    selector.filter(function (elem) {
      var _elem$0$bemStructure;

      return (
        elem.length === 1 &&
        ((_elem$0$bemStructure = elem[0].bemStructure) === null ||
        _elem$0$bemStructure === void 0
          ? void 0
          : _elem$0$bemStructure.length) === 1 &&
...
var _default = _stylelint.default.createPlugin(ruleName, function (options) {
  return function (root, result) {
    var unabled = true;

    var validOptions = _stylelint.default.utils.validateOptions(
      result,
      ruleName,
      {
        actual: options,
    ...
    root.walk(function (node) {
      ...
      if (unabled && node.type == 'rule') {
        ...
        var pyramidSelector = selector.map(function (selectorPart) {
          var decomposedSelector = (0, _splitSelector.default)(
            selectorPart,
            options,
          );
        ...
        var pyramidNode = _objectSpread(
          _objectSpread({}, node),
          {},
          {
            pyramidSelector: pyramidSelector,
          },
        );
